...
/**
 * @author zhoujp
 * @version FileServerHandler.java, v 0.1 2020-12-31 10:38 zhoujp
 */
public class FileServerHandler extends ChannelInboundHandlerAdapter {
     ...
     * 文件默认存储地址
     */
    private String fileDir = System.getProperty("user.dir") + File.separator + "upload";
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        if (msg instanceof RequestFile) {
            ...
            if(start == 0){
                ...
                String path = fileDir + File.separator + md5+ef.getFileType();
                file = new File(path);
                fileSize = ef.getFileSize();

                //根据 MD5 和 文件类型 来确定是否存在这样的文件 如果存在就 秒传
                if( file.exists() ) {
                    log.info("file exists:" + ef.getFileName()+"--" +ef.getFileMd5() +"[" + ctx.channel().remoteAddress()+"]");
            ...
            if (byteRead > 0 && (start < fileSize && fileSize != -1)) {
                ...
                log.info("create file success:" +ef.getFileName()+"--" +ef.getFileMd5() +"[" + ctx.channel().remoteAddress() +"]");
    ...
    private String getFilePath(){
        if( file != null ){
            return "http://127.0.0.1/upload/" +"/" + file.getName();
        }else{
            return null ;
        }
    }
